...
export default function Pipeline() {
  ...
  const handleCreateDeal = (e: React.FormEvent) => {
    ...
    const clientObj = clients.find(c => c.company === newDealClient) || clients[0];
    ...
    mockDb.addDeal({
      ...
      targetDate: newDealTargetDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      notes: newDealNotes
    });
...
- Database relational design and UPI checkouts hooks integration.`;
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("invoicehq_proposal_draft_title", `${deal.title} Proposal`);
      ...
      localStorage.setItem("invoicehq_proposal_draft_budget", String(deal.value));
    }

    triggerToast("AI Proposal draft compiled! Redirecting to Proposals generator...");
  ...
  const columns: { key: Deal["stage"]; label: string; color: string }[] = [
    { key: "lead", label: "Leads", color: "border-t-2 border-t-blue-500 bg-blue-500/5 text-blue-600 dark:text-blue-400" },
    { key: "proposal", label: "Proposal", color: "border-t-2 border-t-indigo-500 bg-indigo-500/5 text-indigo-600 dark:text-indigo-400" },
    { key: "negotiation", label: "Negotiation", color: "border-t-2 border-t-purple-500 bg-purple-500/5 text-purple-600 dark:text-purple-400" },
    { key: "won", label: "Won (Closed)", color: "border-t-2 border-t-state-success bg-state-success/5 text-state-success" },
    { key: "lost", label: "Lost", color: "border-t-2 border-t-state-danger bg-state-danger/5 text-state-danger" }
  ];

  return (
    <WorkspaceLayout title="Sales Pipeline" subtitle="Manage deals and track client conversions">
      {/* Toast Notification */}
      {toastMsg && (
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface-card border border-border-line p-4 rounded-2xl shadow-sm">
          ...
          <div className="flex items-center gap-2 bg-surface-bg p-1 rounded-xl border border-border-line self-start">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all focus-ring-indigo cursor-pointer ${activeTab === "kanban"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all focus-ring-indigo cursor-pointer ${activeTab === "grid"
                  ? "bg-surface-card text-text-main shadow-sm"
                  : "text-text-muted hover:text-text-main"
                }`}
            >
        ...
        {activeTab === "kanban" ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-start overflow-x-auto pb-4">
            {columns.map((col) => {
              ...
              return (
                ...
                >
                  {/* Column Header */}
                  <div className={`p-3 rounded-xl mb-4 flex items-center justify-between ${col.color}`}>
                          ...
                          <div className="mt-3 flex items-center justify-between gap-2">
                            <div className="flex-1 h-1 bg-surface-bg border border-border-line rounded-full overflow-hidden">
                              <div
                                ...
                                className={`h-full ${deal.stage === "won"
                                    ? "bg-state-success"
                                    : deal.stage === "lost"
                                      ? "bg-state-danger"
                                      : "bg-brand-primary"
          ...
          <div className="bg-surface-card border border-border-line rounded-2xl shadow-sm overflow-hidden">
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-surface-bg border-b border-border-line text-[10px] font-bold text-text-muted uppercase tracking-wider">
                    <th className="py-4 px-6 font-bold w-1/4">Client Identity</th>
                    ...
                    filteredDeals.map((deal) => (
                      ...
                      >
                        <td className="py-4 px-6">
                          <div className="font-bold text-text-main">{deal.clientName}</div>
                          <div className="text-[10px] text-text-muted font-normal mt-0.5">{deal.contact}</div>
                        </td>
                        <td className="py-4 px-6 font-bold text-text-main group-hover:text-brand-primary transition-colors">
                          {deal.title}
                        </td>
                        <td className="py-4 px-6">
                          <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase ${deal.stage === "won"
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2.5">
                            <span className="tabular-nums font-semibold w-8">{deal.confidence}%</span>
                            <div className="w-16 h-1.5 bg-surface-bg border border-border-line rounded-full overflow-hidden">
                              <div
                                style={{ width: `${deal.confidence}%` }}
                                className={`h-full ${deal.stage === "won" ? "bg-state-success" : "bg-brand-primary"
                                  }`}
                              />
                            </div>
                          </div>
                        ...
                        <td className="py-4 px-6 text-text-muted font-semibold tabular-nums">
                          {new Date(deal.targetDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}
                        </td>
                        <td className="py-4 px-6 text-right">
                          <ChevronRight size={15} className="text-text-muted opacity-0 group-hover:opacity-100 transition-opacity" />
                        </td>
                      </tr>
                    ))
                  )}
            ...
            <div className="px-6 py-4 bg-surface-bg border-t border-border-line flex items-center justify-between text-[11px] text-text-muted font-medium">
              ...
              <span className="font-bold">Total pipeline: ₹{filteredDeals.reduce((sum, d) => sum + d.value, 0).toLocaleString("en-IN")}</span>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-surface-card border border-border-line rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
              <div className="px-6 py-4 border-b border-border-line flex items-center justify-between">
                <h3 className="text-sm font-bold text-text-main">Log New Opportunity</h3>
                <button
        ...
        {selectedDeal && (
          <>
            ...
            <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-xs z-40" onClick={() => setSelectedDeal(null)} />
            ...
            <div className="fixed top-0 right-0 h-full w-full max-w-md bg-surface-card border-l border-border-line z-50 shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-250 select-none">
              ...
              <div className="p-6 border-b border-border-line flex items-center justify-between">
                <div>
                  ...
                  <h3 className="text-sm font-bold text-text-main line-clamp-1">{selectedDeal.title}</h3>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-1 bg-surface-bg p-1 rounded-xl border border-border-line">
                    {(["lead", "proposal", "negotiation", "won", "lost"] as Deal["stage"][]).map((stg) => {
                      const active = selectedDeal.stage === stg;
                      return (
                        <button
                          key={stg}
                          onClick={() => handleMoveStage(selectedDeal.id, stg)}
                          className={`py-2 rounded-lg text-[9px] font-bold uppercase transition-all cursor-pointer ${active
                              ? stg === "won"
                ...
                <div className="space-y-4 pt-4 border-t border-border-line">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-bold text-text-main">{selectedDeal.clientName}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-text-main tabular-nums">
                      {new Date(selectedDeal.targetDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-semibold text-text-muted tabular-nums">
                      {new Date(selectedDeal.createdAt).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border-line space-y-2">
                  ...
                  <div className="p-3.5 rounded-xl bg-surface-bg/60 border border-border-line text-xs leading-relaxed text-text-main italic font-medium">
                    &ldquo;{selectedDeal.notes || "No additional logs entered."}&rdquo;
                  </div>
                </div>
                  ...
                  <div className="pt-2">
                    <div className="p-4 bg-state-success/5 border border-state-success/15 rounded-xl space-y-3 animate-in fade-in duration-200">
                      <div className="flex gap-2">
                        <Sparkles size={15} className="text-state-success shrink-0 mt-0.5 animate-pulse" />
                        <p className="text-[11px] text-state-success font-semibold leading-normal">
                          Success! This deal is WON. Now you can draft the scope and structure a project proposal template automatically using AI.
                        </p>
                      </div>
                      <button
                        onClick={() => handleTriggerAIDraftScope(selectedDeal)}
