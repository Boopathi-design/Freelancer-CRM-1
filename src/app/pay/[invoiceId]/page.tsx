...
export default function PayInvoicePage() {
  ...
  const handlePay = () => {
    ...
    setTimeout(() => {
      const result = mockDb.triggerPaymentWebhook(
        invoice.id,
        "UPI",
        `pay_${Date.now().toString(36).toUpperCase()}`,
      );
  ...
  const subtotal =
    invoice?.items.reduce((sum, item) => sum + item.qty * item.rate, 0) ?? 0;
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-md w-full rounded-3xl border border-border-line bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-text-muted">
            The payment link you opened is invalid or has expired.
          </p>
          <button
            onClick={() => router.push("/")}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-text-main">
                  {settings?.companyName ?? "ARK Design Studio"}
                </h1>
                <p className="text-xs text-text-muted">
                  {settings?.portalSubdomain ?? "arkdesign"}.invoicehq.in
                </p>
              </div>
            </div>
            ...
            <section className="space-y-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-black text-text-main">
                    {invoice.id}
                  </h2>
                </div>
              ...
              {!shouldShowSuccess ? (
                <div className="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm">
                  ...
                  <div className="mt-4 text-center text-[11px] text-text-muted">
                    No hidden fees • Secure gateway • Instant confirmation
                  </div>
                </div>
              ) : (
                <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
                  ...
                  <div className="mt-4 space-y-3 text-sm text-emerald-900">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold">
                        {new Date(
                          invoice.paymentReceivedAt ?? Date.now(),
                    ...
                    <div className="flex items-center justify-between">
                      <span>Reference</span>
                      <span className="font-semibold">
                        {invoice.paymentReference ?? "pay_UPI"}
                      </span>
                    </div>
                  </div>
