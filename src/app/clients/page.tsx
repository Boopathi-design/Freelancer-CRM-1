...
function ClientsIndexContent() {
  ...
  return (
    <WorkspaceLayout title="Clients" subtitle="Manage billing relationships and customer details">
      {/* Toast Alert */}
      {toastMsg && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white px-5 py-3 rounded-2xl shadow-2xl border border-brand-primary/30 text-xs font-semibold flex items-center gap-2.5 animate-in slide-in-from-bottom-5 duration-200">
          <Sparkles size={14} className="text-brand-primary animate-pulse shrink-0" />
          <span>{toastMsg}</span>
        </div>
      )}
      ...
      <div className="space-y-6">
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-[9px] font-bold text-text-muted uppercase tracking-wider">{stat.label}</p>
                  <p className="text-sm font-black text-text-main tabular-nums truncate">{stat.value}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-surface-card border border-border-line p-4 rounded-2xl shadow-sm">
          <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-surface-bg border border-border-line text-xs font-semibold text-text-muted w-full md:max-w-xs">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="text-text-muted hover:text-text-main transition-colors">
                <X size={11} />
              </button>
            )}
          </div>
        ...
        <div className="bg-surface-card border border-border-line rounded-2xl shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-border-line text-xs font-medium">
                {isLoading ? (
                  <tr>
                    <td colSpan={7} className="py-12 text-center text-text-muted animate-pulse">
                  ...
                  <tr>
                    <td colSpan={7} className="py-16 text-center">
                      ...
                      <p className="text-text-muted font-semibold">No clients found</p>
                      <p className="text-[10px] text-text-muted/70 mt-1">
                        {searchQuery ? "Try a different search term" : 'Click “Add Client” to get started'}
                      </p>
                    </td>
                  </tr>
                ) : (
                      ...
                      >
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`font-bold transition-colors ${isSelected ? "text-brand-primary" : "text-text-main group-hover:text-brand-primary"}`}>
                              {client.company}
                            </span>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <div className="font-semibold text-text-main">{client.contact}</div>
                          <div className="text-[10px] text-text-muted font-normal mt-0.5">{client.email}</div>
                        </td>
                        <td className="py-4 px-6 text-text-muted font-semibold tabular-nums">{client.gstin}</td>
                        <td className="py-4 px-6 text-right font-extrabold tabular-nums">
                          {client.outstanding > 0 ? (
                            <span className="text-state-danger">₹{client.outstanding.toLocaleString("en-IN")}</span>
                          ) : (
                            <span className="text-state-success">Nil</span>
                          )}
                        </td>
                        <td className="py-4 px-6">
                          <span className={`text-xs font-bold ${getHealthColor(score)}`}>
                            {score}
          ...
          <div className="px-6 py-3 bg-surface-bg border-t border-border-line text-[11px] text-text-muted font-medium flex items-center justify-between">
            <span>Showing {filteredClients.length} of {clients.length} registered clients</span>
            {searchQuery && (
              <button
            ...
            <div className="w-full max-w-md bg-surface-card border border-border-line rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
              <div className="px-6 py-4 border-b border-border-line flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <h3 className="text-sm font-bold text-text-main">New Client Profile</h3>
                </div>
                <button
              ...
              <form onSubmit={handleAddClient} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <Shield size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                    <input
                      type="text"
                      required
                      maxLength={15}
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-text-muted">360° Client Profile</span>
                    </div>
                  </div>
                  <button
                    onClick={() => setSelectedClientId(null)}
                  ...
                  return (
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className={`text-xs font-black ${getHealthColor(score)}`}>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 rounded-xl bg-surface-bg border border-border-line">
                    ...
                    <span
                      className={`text-base font-black tabular-nums ${
                        activeClient.outstanding > 0 ? "text-state-danger" : "text-state-success"
                      }`}
                    >
                    ...
                    {activeClient.outstanding > 0 ? (
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-[9px] text-state-danger">Requires follow-up</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1 mt-1">
                        <CheckCircle size={9} className="text-state-success" />
                        <span className="text-[9px] text-state-success">Fully cleared</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-surface-bg border border-border-line">
                    ...
                    <span className="text-base font-black text-text-main tabular-nums">
                      ...
                      <span className="text-xs font-medium text-text-muted ml-1">days</span>
                    </span>
                    <div className="flex items-center gap-1 mt-1">
                      {activeClient.avgDelay <= 7 ? (
                        <>
                          <TrendingDown size={9} className="text-state-success" />
                          <span className="text-[9px] text-state-success">Prompt payer</span>
                        </>
                      ) : (
                        <>
                          <Clock size={9} className="text-state-warning" />
                          <span className="text-[9px] text-state-warning">Moderate delay</span>
                        </>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2.5">
                    {[
                      { icon: Building2, label: "Company", value: activeClient.company },
                      { icon: Phone, label: "Contact Person", value: activeClient.contact },
                      { icon: Mail, label: "Email", value: activeClient.email },
                      { icon: Shield, label: "GSTIN", value: activeClient.gstin, mono: true },
                    ].map((row) => {
                      const Icon = row.icon;
                      return (
                        <div
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            <p className={`text-xs text-text-main font-semibold truncate ${row.mono ? "font-mono tabular-nums" : ""}`}>
                              {row.value}
                            </p>
                          </div>
                        </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {[
                      { label: "Total Invoices", value: `${activeClient.invoiceCount} generated` },
                      {
                        label: "Last Invoice",
                        value:
                          activeClient.lastInvoiceDate === "—"
                            ? "None yet"
                            : new Date(activeClient.lastInvoiceDate).toLocaleDateString("en-IN", {
                                day: "2-digit",
                                month: "short",
                                year: "numeric",
                              }),
                      ...
                      <div key={item.label} className="p-3 rounded-xl bg-surface-bg border border-border-line">
                  ...
                  <div className="space-y-2.5">
                    <div className="p-3.5 bg-surface-bg border border-border-line rounded-xl">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] font-bold text-text-main">Invoice Dispatched</span>
                        <span className="text-[9px] text-text-muted">3 days ago</span>
                      </div>
                      <p className="text-[10px] text-text-muted leading-relaxed">
                        Payment link and invoice statement INV-2024-041 sent to {activeClient.email}.
                      </p>
                    </div>
                    <div className="p-3.5 bg-surface-bg border border-border-line rounded-xl">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[10px] font-bold text-text-main">Invoice Viewed</span>
                        <span className="text-[9px] text-text-muted">5 days ago</span>
                      </div>
                      <p className="text-[10px] text-text-muted leading-relaxed">
                        {activeClient.contact} viewed INV-2024-037. Flagged for finance reconciliation.
                      </p>
                    </div>
                  </div>
                </div>
