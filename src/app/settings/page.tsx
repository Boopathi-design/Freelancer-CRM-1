...
const gstSlabs = [
  ...
  { rate: "5%", category: "Essential Services", example: "Basic food, transport" },
  { rate: "12%", category: "Standard Services", example: "Hospitality, software" },
  { rate: "18%", category: "Professional Services", example: "IT/Design consulting, SaaS" },
  { rate: "28%", category: "Luxury / Premium", example: "Premium entertainment" },
];
  ...
  return (
    ...
    >
      ...
      <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-brand-primary/10 via-brand-primary/5 to-transparent border border-brand-primary/15 flex items-center gap-4">
        ...
        <div>
          <h2 className="text-sm font-bold text-text-main">Workspace Preferences</h2>
          <p className="text-xs text-text-muted mt-0.5">
            Manage your company identity, design tokens, GST configuration, and billing plan.
          </p>
        </div>
        ...
        <nav className="w-full lg:w-52 shrink-0 bg-surface-card border border-border-line rounded-2xl p-2 shadow-sm">
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                <Icon size={15} className={isActive ? "text-brand-primary" : ""} />
                <span>{tab.label}</span>
        ...
        <div className="flex-1 min-w-0 space-y-6">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="bg-surface-card border border-border-line rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2.5 pb-4 border-b border-border-line mb-6">
                  ...
                  <h3 className="text-sm font-bold text-text-main">Company Identity</h3>
                </div>
                ...
                <form onSubmit={handleSave} className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <Globe size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                        <input
                          type="text"
                          value={placeOfSupply}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  { label: "Plan", value: "Pro Workspace", icon: Sparkles, color: "text-brand-primary" },
                  { label: "Currency", value: baseCurrency || "INR", icon: CreditCard, color: "text-state-success" },
                  { label: "Supply State", value: placeOfSupply || "—", icon: Globe, color: "text-state-warning" },
                  { label: "GSTIN Status", value: gstin ? "Active" : "Not Set", icon: Shield, color: gstin ? "text-state-success" : "text-state-danger" },
                ].map((card) => {
                  const Icon = card.icon;
                  return (
                    <div key={card.label} className="bg-surface-card border border-border-line rounded-2xl p-4 shadow-sm">
                      ...
                      <p className="text-xs font-bold text-text-main mt-0.5 truncate">{card.value}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="bg-surface-card border border-border-line rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2.5 pb-4 border-b border-border-line mb-6">
                  ...
                  <h3 className="text-sm font-bold text-text-main">Design Token Tree</h3>
                ...
                <p className="text-xs text-text-muted mb-5 leading-relaxed">
                  These CSS custom properties define the entire visual language of InvoiceHQ. They cascade through both light and dark themes automatically.
                </p>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[10px] font-bold text-text-main font-mono">{tok.name}</span>
                            <button
                            ...
                            >
                              {copiedToken === tok.name ? (
                                <CheckCircle size={11} className="text-state-success" />
                              ) : (
                                <Copy size={11} />
                              )}
                            </button>
                          </div>
                          <p className="text-[9px] text-text-muted leading-tight mb-2">{tok.desc}</p>
                          <div className="flex items-center gap-4 text-[9px] font-medium">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-text-main font-mono">{tok.light}</span>
                              <span className="text-text-muted">{tok.lightLabel}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <div
                                className="w-3 h-3 rounded-full border border-border-line shadow-sm"
                              ...
                              <span className="text-text-main font-mono">{tok.dark}</span>
                              <span className="text-text-muted">{tok.darkLabel}</span>
                            </div>
                          </div>
                        </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="bg-surface-card border border-border-line rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2.5 pb-4 border-b border-border-line mb-6">
                  ...
                  <h3 className="text-sm font-bold text-text-main">GST Configuration</h3>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-center justify-between p-4 bg-surface-bg rounded-xl border border-border-line">
                    <div>
                      <p className="text-xs font-bold text-text-main">Auto-apply GST on Invoices</p>
                      <p className="text-[10px] text-text-muted mt-0.5">
                        Automatically calculate and append GST breakdown on all new invoices
                      </p>
                    </div>
                    <button
              ...
              <div className="bg-surface-card border border-border-line rounded-2xl p-6 shadow-sm">
                <div className="flex items-center gap-2.5 pb-4 border-b border-border-line mb-4">
                  ...
                  <h3 className="text-sm font-bold text-text-main">GST Slab Reference</h3>
                  ...
                  {gstSlabs.map((slab) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-text-main">{slab.category}</p>
                        <p className="text-[10px] text-text-muted mt-0.5">{slab.example}</p>
                      </div>
                      {defaultGstRate === slab.rate.replace("%", "") && (
                        <CheckCircle size={14} className="text-brand-primary ml-auto shrink-0" />
                      )}
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "billing" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-brand-primary to-indigo-700 rounded-2xl p-6 text-white shadow-xl shadow-brand-primary/25">
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-white/80">Current Plan</span>
                  </div>
                  <h3 className="text-2xl font-black mb-1">Pro Workspace</h3>
                  <p className="text-sm text-white/70 mb-5">Full access · Unlimited invoices · GST automation</p>
              ...
              {/* Feature Matrix */}
              <div className="bg-surface-card border border-border-line rounded-2xl p-6 shadow-sm">
                <h3 className="text-sm font-bold text-text-main mb-5">Plan Features</h3>
                <div className="space-y-3">
                  {[
                    ...
                    { feature: "Team Collaboration (5 seats)", included: false },
                    { feature: "API Access & Webhooks", included: false },
                  ].map((item) => (
                    <div key={item.feature} className="flex items-center gap-3">
                      <div
                        className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${
                          item.included ? "bg-state-success/15" : "bg-surface-bg border border-border-line"
                        }`}
                      >
                        {item.included && <CheckCircle size={10} className="text-state-success" />}
                      </div>
                      <span
                        className={`text-xs font-semibold ${
                          item.included ? "text-text-main" : "text-text-muted line-through"
                        }`}
                      >
                        {item.feature}
                      </span>
